...
#
# Outputs:
#   - plot_trend: Overall adjusted vs observed prevalence plot
#   - plot_by_gender: Gender-stratified trend plot
#   - trend_results: List containing key statistical results
#
# Usage in Quarto:
...
df_can <- DTT[lypPop_cannabis == 1]  # Keep only eligible respondents
...
plot_trend <- highchart() %>%
  ...
  hc_title(
    text = "Figur 6: Andel cannabisbruk siste 12 måneder: 2013 - 2025",
...
plot_by_gender <- highchart() %>%
  ...
  hc_title(
    text = "Prevalens av cannabisbruk etter kjønn over tid",
    style = list(fontWeight = "bold", fontSize = "16px")
  ) %>%
  hc_subtitle(
...
#
# Available objects:
#   - plot_trend: Overall prevalence plot
#   - plot_by_gender: Gender-stratified plot
#   - trend_results: List with all statistical results
#
...
#
#   Access results:
#   trend_results$overall$year_pvalue
#   trend_results$males$significant
################################################################################
