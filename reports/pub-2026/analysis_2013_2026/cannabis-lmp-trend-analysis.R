...
#
# Outputs:
#   - plot_trend_lmp: Overall adjusted vs observed prevalence plot
#   - plot_by_gender_lmp: Gender-stratified trend plot
#   - trend_results_lmp: List containing key statistical results
#
# Usage in Quarto:
#   source("cannabis_trend_analysis.R")
#   print(plot_trend_lmp)
#   print(plot_by_gender_lmp)
################################################################################
...
df_can <- DTT[lmpPop_cannabis == 1]  # Keep only eligible respondents
...
plot_by_gender_lmp <- highchart() %>%
  ...
  hc_title(
    text = "Prevalens av cannabisbruk siste 4 uker etter kjønn over tid",
    style = list(fontWeight = "bold", fontSize = "16px")
  ) %>%
  hc_subtitle(
...
#
# Available objects:
#   - plot_trend_lmp: Overall prevalence plot
#   - plot_by_gender_lmp: Gender-stratified plot
#   - trend_results_lmp: List with all statistical results
#
...
#
#   Access results:
#   trend_results_lmp$overall$year_pvalue
#   trend_results_lmp$males$significant
################################################################################
